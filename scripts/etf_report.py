...
def _get_or_create_category_robust(url: str, headers: dict, name: str) -> int | None:
    ...
    try:
        # 1순위: slug 기반 조회 (가장 안정적 — 한국어 URL인코딩 충돌 없음)
        if slug:
            resp = requests.get(
            ...
            if cats:
                log.info(f"카테고리 조회 성공 (slug): {name} → id={cats[0]['id']}")
                return cats[0]["id"]
        ...
        if resp.ok:
            for c in resp.json():
                if _html.unescape(c["name"]).lower() == name.lower():
                    cat_id = c["id"]
                    # 한국어 인코딩 slug → 영문 slug 자동 교정
                    if slug and c.get("slug", "").startswith("%"):
                        try:
                            requests.post(
                                f"{url}/wp-json/wp/v2/categories/{cat_id}",
                                headers=headers,
                                json={"slug": slug},
                                timeout=10,
                            )
                            ...
                            pass
                    log.info(f"카테고리 조회 성공 (name): {name} → id={cat_id}")
                    return cat_id

        # 3순위: 신규 생성 — 항상 영문 slug 포함
        ...
        if resp.ok:
            ...
            log.info(f"카테고리 신규 생성: {name} (slug={slug}) → id={cat_id}")
            return cat_id
        else:
            log.warning(f"카테고리 생성 실패: {resp.status_code} {resp.text[:200]}")
