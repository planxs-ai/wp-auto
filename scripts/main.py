...
class DynamicKeywordGenerator:
    ...
    def __init__(self, site_niches=None, site_id=""):
    ...
    def _get_dashboard_niches(self):
        """사이트별 dashboard_config에서 선택된 니치 가져오기"""
        cfg = _get_dashboard_config(self.site_id)
        if cfg:
    ...
    def generate(self, count=5, fallback=False):
        ...
        """
        # 사이트별 대시보드 설정 1회 조회
        dashboard_cfg = _get_dashboard_config(self.site_id)

        # autoMode 체크 (폴백 모드에서는 스킵 — 키워드 없으면 무조건 생성해야 함)
        if not fallback:
            if dashboard_cfg.get("autoMode") is False:
                log.info("  autoMode OFF — 정적 키워드 사용")
                return None

        niches = dashboard_cfg.get("niches", [])
        if not niches:
            if fallback:
    ...
    def _call_ai(self, prompt):
        ...
        # 2순위: Grok (DeepSeek 대체 — hex 환각 이슈로 승격)
        if GROK_KEY:
            try:
                resp = requests.post(
        ...
        # 3순위: DeepSeek (비상 폴백 — ALLOW_DEEPSEEK=1일 때만)
        if DEEPSEEK_KEY and os.environ.get("ALLOW_DEEPSEEK") == "1":
            try:
                resp = requests.post(
...
1. 제목: <title> 태그. 키워드를 앞부분에 자연스럽게 포함. 전문적+검색 친화적
   ...
   - **본문 전체에서도 금지**: 12자리 이상 영숫자 해시(예: 5828b285fbf3), "고유 데이터 코드", "식별 코드", UUID, 세션 ID, 트랜잭션 ID 등 내부/가짜 식별자를 절대 생성하거나 언급하지 말 것. 실제 제품명/종목코드가 아닌 한 hex 문자열은 무조건 금지.
...
- 성인, 선정적, 데이팅 앱 관련 내용 (단, "만 19세 이상 성인", "성인 평균" 같은 **인구통계 용어**는 허용)
- 주류, 담배, 마약, 약물 관련 내용
- 무기, 폭력, 잔인한 묘사
...
3. **Authoritativeness (권위)**: 데이터 출처 인용 최소 3곳
   ...
   - 공신력 있는 기관 (KRX, KCB, 통계청, 금융감독원)
   - 각 수치마다 "(출처: [기관명], 2025년 12월 기준)" 형식

4. **Trustworthiness (신뢰)**: 데이터 출처/수치 근거만 본문에 명확히 표기
   - (참고: 면책 고지, 저자 Author Box, Article JSON-LD Schema는 발행 파이프라인에서 사이트별로 자동 삽입되므로 프롬프트 출력에는 포함하지 말 것. 본문 끝에 <div class="author-box">, <div class="disclaimer">, <script type="application/ld+json"> 블록을 AI가 직접 생성하지 않습니다.)
...
첫 문장에서 통념을 제시하고, 두 번째 문장에서 데이터로 즉시 반박하십시오.""",
    ...
    """도입부 패턴: [수치 폭탄]
실존하는 공개 통계가 있을 때만 그 수치로 시작하고, 반드시 출처 기관을 함께 밝히십시오. 예: "OO청에 따르면 국내 ~시장 규모가 130조 원을 넘어섰습니다."
→ 검증 가능한 수치만 사용합니다. 출처가 불확실하면 "최근 빠르게 커지고 있습니다" 같은 정성적 표현으로 대체하십시오.
가짜 퍼센트/비율을 지어내지 말 것. 수치를 쓰면 본문에 해당 기관 공식 링크를 포함하십시오.""",
...
반드시 과거 수치와 현재 수치를 병렬 제시하여 변화의 크기를 체감시키십시오.""",
    ...
    # 4. 실패 시나리오 — 손실 회피 심리
    """도입부 패턴: [실패 시나리오]
"준비 없이 ~를 시작하면 흔히 ~한 시행착오를 겪습니다."처럼, 흔한 실수를 정성적으로 묘사하며 시작하십시오.
→ 손실 회피 심리(Loss Aversion)를 자극하되, "73%의 확률" 같은 출처 없는 통계는 절대 지어내지 마십시오.
실패의 비용은 출처가 있을 때만 수치로, 없으면 "적지 않은 시간과 비용"처럼 정성적으로 제시하십시오.""",
...
9-1. **권위 출처 하이퍼링크 필수 (E-E-A-T)**: 통계·법령·제도·세율 등을 인용할 때는 해당 공식 기관 페이지로 연결되는 <a> 링크를 본문에 최소 1~2개 포함할 것.
   ...
   - 형식: <a href="https://www.nts.go.kr" target="_blank" rel="noopener nofollow">국세청</a>
   - 존재가 불확실한 구체 URL은 만들지 말고 기관 메인 도메인으로 링크할 것.
...
6. 핵심 주장은 가능하면 비교·근거로 뒷받침하되, 출처 없는 수치를 새로 지어내지 말 것 (근거 없이 떠도는 단정 문장은 삭제)
...
class ContentGenerator:
    """멀티모델 AI 글 생성기 — Grok→Gemini (초안) + Claude (폴리싱). DeepSeek 제외 (hex 환각 이슈)."""
    ...
    def generate(self, keyword, intent="informational", category="",
        ...
        for model_id in model_chain:
            if draft:
                break
            if model_id in ("grok", "grok-3", "grok-3-mini", "grok-4") and GROK_KEY:
...
class AffiliateManager:
    def __init__(self, site_cfg=None, global_cfg=None):
        self.links_file = DATA / "affiliates.json"
        self.links = self._load()
        # site_cfg(사이트별 대시보드 설정) 우선, global_cfg(레거시 호환) 폴백
        self.site_cfg = site_cfg or global_cfg or {}
        self.manual_coupang = self.site_cfg.get("coupang_manual_products", [])
        self.tenping_campaigns = self.site_cfg.get("tenping_campaigns", [])
...
def _sanitize_content(content):
    """본문에서 해시값, 기술적 코드 등 AI 환각 오염 패턴 제거.

    예: '고유 데이터 코드 5828b285fbf3을 분석한', '5828b285fbf3 맞춤 포트폴리오'
    AI(DeepSeek/Claude)가 가짜 '데이터 코드'로 12자리 hex 문자열을 생성하는 케이스 방지.
    """
    import re as _re
    if not content:
        return content

    # 1) "고유 데이터 코드 <hex>을/를/이/가 분석/공개/기반" 류 문장/구문 제거
    content = _re.sub(
        r'(고유\s*)?(데이터|식별|분석|고객|사용자|프로필|유저)?\s*코드\s*[:：]?\s*[0-9a-f]{10,}[을를이가은는]?\s*',
        '', content, flags=_re.IGNORECASE
    )
    # 2) "<hex> 맞춤/기반/데이터로" 같이 문장 앞에 해시가 붙은 케이스
    content = _re.sub(r'\b[0-9a-f]{10,}\b\s*(맞춤|기반|데이터로|전용|코드)', r'\1', content)
    # 3) 괄호/대괄호 안 hex 해시 제거
    content = _re.sub(r'[\(\[\{]\s*[0-9a-f]{10,}\s*[\)\]\}]', '', content)
    # 4) 단독 12자리 이상 hex 해시 제거 (URL/클래스명/color hex는 #/- 포함이라 영향 적음)
    content = _re.sub(r'(?<![#\-/\w])[0-9a-f]{12,}(?![\w\-/])', '', content)
    # 5) UUID 형식 제거 (8-4-4-4-12)
    content = _re.sub(r'\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b', '', content)
    # 6) "세션/요청/트랜잭션 ID" 류 노출 제거
    content = _re.sub(r'(세션|요청|트랜잭션|트래킹|추적)\s*(ID|아이디)\s*[:：]?\s*[0-9a-zA-Z\-]{8,}', '', content)
    # 7) 정리: 빈 괄호, 연속 공백, 끊어진 조사 정리
...
# ── 사이트별 E-E-A-T 메타 (저자/아바타/브랜드) ──
# 도메인 또는 SITE_ID 기준 매핑. Supabase sites.config.author로도 오버라이드 가능.
SITE_AUTHORS = {
    "bomissu.com": {
        "name": "Bomissu 운영자",
    ...
    "planx-ai.com": {
        "name": "PlanX 투자리서치팀",
        "url": "https://planx-ai.com/about/",
        "avatar": "https://planx-ai.com/wp-content/uploads/2026/04/planx-avatar.png",
        "bio": "퀀트 전략·ETF 모멘텀·섹터 로테이션 분석 전문",
        "publisher": "PlanX AI",
    },
}
...
def _make_initial_avatar_data_uri(name):
    """이름 첫 글자로 Gold 그라데이션 SVG 아바타 생성. base64 data URI 반환."""
    import base64 as _b64
    initial = (name or "B").strip()[:1].upper() if name else "B"
    # 영문이 아니면 첫 한글 글자 사용
    svg = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" width="128" height="128"><defs><linearGradient id="g" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#FFE8CC"/><stop offset="100%" stop-color="#D4A853"/></linearGradient></defs><rect width="128" height="128" rx="64" fill="url(#g)"/><text x="64" y="82" font-family="Georgia,serif" font-size="60" font-weight="700" text-anchor="middle" fill="#1A1612">{initial}</text></svg>'''
    b64 = _b64.b64encode(svg.encode("utf-8")).decode("ascii")
    return f"data:image/svg+xml;base64,{b64}"


def _resolve_site_author(site_config=None):
    """도메인/SITE_ID/site_config 기반 저자 정보 반환.
    우선순위:
      1) site_config.config.author (명시적 저자 설정 — 하드코딩 가능)
      2) site_config.config.blog_owner (대시보드 settings 입력값) — 고객 fork 시 자동 적용
      3) SITE_AUTHORS[domain] (bomissu/planx-ai 내장)
      4) DEFAULT_AUTHOR
    """
    cfg = (site_config or {}).get("config") or {}

    # 1) 명시적 author 설정 우선
    custom = cfg.get("author") or {}
    if custom.get("name"):
        avatar = custom.get("avatar") or _make_initial_avatar_data_uri(custom["name"])
        return {
            "name": custom["name"],
            "url": custom.get("url", ""),
            "avatar": avatar,
            "bio": custom.get("bio", "정보성 분석 전문"),
            "publisher": custom.get("publisher", custom["name"]),
        }

    # 2) 대시보드 settings 필드 (blog_owner / blog_desc / contact_email) 자동 매핑
    blog_owner = cfg.get("blog_owner") or ""
    blog_desc = cfg.get("blog_desc") or ""
    if blog_owner:
        return {
            "name": blog_owner,
            "url": (site_config or {}).get("wp_url", "").rstrip("/") + "/about-us/" if (site_config or {}).get("wp_url") else "",
            "avatar": _make_initial_avatar_data_uri(blog_owner),
            "bio": blog_desc or "정보성 분석 전문 블로그",
            "publisher": blog_owner,
        }

    # 3) 내장 사이트 매핑 (bomissu / planx-ai)
    url = (site_config or {}).get("wp_url") or WP_URL or ""
    from urllib.parse import urlparse
    host = urlparse(url).netloc.replace("www.", "") if url else ""
    if host in SITE_AUTHORS:
        return SITE_AUTHORS[host]

    # 4) SITE_ID 폴백
    sid = (site_config or {}).get("id") or SITE_ID or ""
    for domain, author in SITE_AUTHORS.items():
        if sid and sid.replace("-", "") in domain.replace(".", "").replace("-", ""):
            return author
    return DEFAULT_AUTHOR


def _inject_eeat_blocks(content, title, site_config=None, slug="", pub_date=""):
    """발행 직전 파이프라인 후처리: Author Box / Disclaimer / Article Schema 자동 주입.
    이미 존재하면 중복 삽입 안 함."""
    ...
    author = _resolve_site_author(site_config)
    today = pub_date or _dt.now(KST).strftime("%Y-%m-%d")
    today_kr = _dt.now(KST).strftime("%Y년 %m월 %d일")
    ...
    if "class=\"author-box\"" not in content:
        author_box = (
            ...
            f"<img src=\"{author['avatar']}\" alt=\"{author['name']}\" style=\"width:64px;height:64px;border-radius:50%;flex-shrink:0\"/>"
            "<div>"
    ...
    # Disclaimer (Author Box 앞)
    if "class=\"disclaimer\"" not in content:
        disc = (
            ...
            f"구체적 결정은 전문가 상담 후 진행하세요. 최종 업데이트: {today_kr}</div>\n"
        )
        if "class=\"author-box\"" in content:
            content = content.replace("<div class=\"author-box\"", disc + "<div class=\"author-box\"", 1)
        else:
            content = content.rstrip() + disc
    ...
    if "\"@type\":\"Article\"" not in content and "\"@type\": \"Article\"" not in content:
        ...
        desc = plain[:180]
        publisher = author.get("publisher") or author.get("name")
        url = author.get("url") or ""
        avatar = author.get("avatar") or ""
        site_url = (site_config or {}).get("wp_url") or WP_URL or ""
        from urllib.parse import urlparse
        host = urlparse(site_url).netloc
        article_url = f"https://{host}/{slug}/" if host and slug else site_url
        schema = {
        ...
        js = _json.dumps(schema, ensure_ascii=False)
...
def _get_dashboard_config(site_id):
    """사이트별 dashboard_config.config 조회. 없으면 {} 반환."""
...
def should_run_now(site_config=None):
    ...
    # 사이트별 스케줄 우선 (sites.config → dashboard_config → 폴백)
    settings = {}
    site_cfg = (site_config or {}).get("config") or {}
    if site_cfg.get("daily_count") and site_cfg.get("schedule_times"):
        settings = site_cfg
    else:
        # 사이트별 dashboard_config에서 스케줄 조회
        site_id = (site_config or {}).get("id") or SITE_ID
        dashboard_cfg = _get_dashboard_config(site_id)
        if dashboard_cfg:
            settings = dashboard_cfg
...
def run_pipeline(count=5, dry_run=False, pipeline="autoblog", site_override=None, adsense_mode=False, golden_mode=False, cli_draft_model="", cli_polish_model="", niches=None):
    ...
    # 사이트별 대시보드 설정 로드 (dashboard_config.config)
    site_dashboard_cfg = _get_dashboard_config(SITE_ID)

    site_cfg = (site_config or {}).get("config") or {}
    ai_cfg = (site_config or {}).get("ai_config") or {}

    # 유효 설정: CLI > 사이트DB(sites.config) > 대시보드(dashboard_config) > 기본값
    effective_lang = site_cfg.get("lang", site_dashboard_cfg.get("lang", "ko"))
    ...
    # Stage 기반 설정 (사이트별 monetization_stage)
    monetization_stage = int(site_dashboard_cfg.get("monetization_stage", site_cfg.get("monetization_stage", 1)))
    ...
    if not (GROK_KEY or GEMINI_KEY or DEEPSEEK_KEY):
    ...
    dkg = DynamicKeywordGenerator(site_niches=_site_niches, site_id=SITE_ID)
    cg = ContentGenerator()
    cf = ContentFormatter()
    im = ImageManager()
    am = AffiliateManager(site_cfg=site_dashboard_cfg)
    ao = AdSenseOptimizer()
    qg = QualityGate()
    ...
    for i, kw_data in enumerate(keywords, 1):
        ...
        if not passed:
            for retry in range(2):
                ...
                if content2:
                    ...
                    content2 = _sanitize_content(content2)
        ...
        # Step 6.8: E-E-A-T 블록 자동 주입 (사이트별 Author Box / Disclaimer / Article Schema)
        try:
            content = _inject_eeat_blocks(
                content, title,
                site_config=site_override,
