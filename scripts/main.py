...
6. 마무리: "이 글에서 얻은 것" 핵심 3~5줄 + FAQ 3개
6. 톤: 한국 독자 맞춤 — 신뢰감 있으면서 쉽고 친근하게
...
class ContentFormatter:
    ...
    def _style_table(self, content):
        ...
        for m in reversed(tables):
            ...
            table_inner = re.sub(
                r'<thead[^>]*>',
                f'<thead style="background:{accent}">',
                table_inner, flags=re.IGNORECASE
            )

            # Step 3: th 스타일 (흰 글씨) — (?!ead)로 <thead> 제외
            table_inner = re.sub(
                r'<th(?!ead)(?:\s[^>]*)?>',
                '<th style="padding:14px 16px;color:#ffffff;font-weight:700;text-align:left;font-size:14px">',
                table_inner, flags=re.IGNORECASE
            )
    ...
    def _style_strong(self, content):
        """<strong> 태그에 연한 노랑 밑줄 하이라이트"""
        def _replace_strong(m):
            tag = m.group(0)
            if 'style=' in tag:
                return tag
            return '<strong style="color:#1a1a2e;background:linear-gradient(transparent 60%,#fef9c3 60%)">'
