...
## 후속 실측 및 추가 적용 — 2026-09-13 KST

- 사용자가 WPCode 스니펫 1438을 활성화한 뒤 홈 요약 10개, 사이트 안내, 하단 정보 링크를 공개 HTML에서 확인했습니다.
- Breeze·객체 캐시를 비운 뒤 자동차보험 글의 중복 목차 2개가 연결형 1개로 표시됨을 확인했습니다. 315편 전수 검증은 아닙니다.
- `?page_id=1208` 및 기존 한글 정책 URL에서 `/privacy-policy/`로 301 연결을 확인했습니다.
- 비어 있던 IT·테크 리뷰 분류(418)에 재작성한 글 1426, 1429를 분류했습니다. 기존 분류는 두 글 모두 `[1]`(미분류)이었고 현재 `[418]`입니다. 공개 분류 화면에서 글 2편과 요약 표시를 확인했습니다.
- 자동차보험(1430), 탈모 샴푸(1427)를 별도 관리자 원문 백업 후 재작성했습니다. 근거 없는 체험·절약률·효능 순위·진료 지연 안내를 제거하고 확인 기록표와 공식 출처, 정정 안내를 넣었습니다.
- 이 2편은 제목·요약·본문의 인증 재조회 일치, 기존 주소·공개 상태 유지, 공개 화면 정정 안내 표시 및 기존 문제 문구 제거를 확인했습니다. 재작성 누계는 5편이며 전체 700편 검증 완료가 아닙니다.

### 예약 발행 조치 — 아직 미적용

main은 여전히 기존 공개 발행 코드를 사용합니다. PR #3에는 다른 사이트에 영향을 주는 공통 파이프라인 변경이 있어 전체 병합을 진행하지 않았습니다. 별도 PR #4는 원본 저장소의 `publish.yml` 중 planx-ai 예약과 `etf-report.yml` 예약만 건너뛰며 수동 실행·bomissu·통계 수집은 유지합니다. YAML 파싱과 대상 if 외 설정 불변 검증을 완료했습니다.

PR #4 병합은 자동 승인 검토가 명시적인 운영 일정 변경 병합 승인이 없다는 이유로 차단했습니다. 사용자 승인 전 재시도·직접 main 쓰기 등 우회는 하지 않습니다. 따라서 예약 중지나 초안 전환이 운영에 적용됐다고 보고하지 않습니다.

추가 콘텐츠 출처: 손해보험협회 자동차보험 종합포털 및 소비자포털 FAQ, 미국피부과학회(AAD) 탈모 진단·관리 안내. 특정 보험료·치료 성과를 약속하지 않습니다. 실제 개인정보 보유기간, 나머지 글의 근거, AdSense 계정 소유권 확인은 여전히 남아 있습니다.
